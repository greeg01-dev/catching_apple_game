...
pub fn move_basket(
    ...
    if keyboard_input.pressed(KeyCode::Left) {
        basket_transform.translation.x = (basket_transform.translation.x - time.delta_seconds() * 300.0).max(-360.0);
    }
    // move basket when Right Button is pressed
    if keyboard_input.pressed(KeyCode::Right) {
        basket_transform.translation.x = (basket_transform.translation.x + time.delta_seconds() * 300.0).min(360.0);
    }
}